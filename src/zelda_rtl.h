...
/* Esteban fixed camera (wide_camera.c).  Mode 1 = FIXED, 0 = STANDARD. */
void ZeldaSetWidescreenEdgeMode(int mode);
int ZeldaGetWidescreenEdgeMode(void);
int ZeldaGetWidescreenFixedCameraMargin(void);

/* Live aspect switching.  ZeldaSetWidescreen reconfigures the engine between
 * frames; ZeldaConsumeDisplayReconfig tells the frontend exactly once that its
 * texture and layout have to follow. */
void ZeldaSetWidescreen(bool enable);
/* The tri-state form (kAleksAspect_*); ZeldaSetWidescreen is a wrapper. */
void ZeldaSetAspect(int aspect);

/* ALEKS Quick Item: a HUD item id (kHudItem_*) to USE on the next
 * Link_HandleYItem without changing the equipped Y item; 0 when idle. */
extern uint8 g_aleks_quick_use_hud_item;
