...
/* ------------------------------------------------------------------ *
 * Esteban fixed camera, ported from
 * Zelda3-3DS-Reference/app/jni/src/src/zelda_rtl.c + wide_camera.c.
 *
 * In WIDE, vanilla pins the camera to Link, so the extra columns keep running
 * into the area bounds and the picture snaps at every edge.  FIXED holds the
 * camera off the bounds by the widescreen margin and interpolates across
 * horizontal transitions instead.
 *
 * This is the donor's CURRENT implementation including its bail-outs (dungeon
 * map menu, world map, lantern cone) -- those are the fixes its own history
 * added, so they come along rather than being re-derived.
 *
 * Deliberately NOT ported from the same donor file: the 3DS multi-core PPU
 * worker threads.  Switch has the headroom and they would drag in libctru.
 * ------------------------------------------------------------------ */
static int g_widescreen_edge_mode;

/* Set when the aspect changes at runtime so the frontend can resize its
...
/* ------------------------------------------------------------------ *
 * Live aspect switching.  The engine can change its own extra columns between
 * frames; what it cannot do by itself is resize the frontend's texture, so
 * that half is published as a one-shot flag the frontend consumes.
 * ------------------------------------------------------------------ */
void ZeldaSetWidescreen(bool enable) {
  ZeldaSetAspect(enable ? kAleksAspect_Wide : kAleksAspect_4x3);
}

/*
 * The real implementation.  Every aspect goes through here, and the amount of
 * side space is asked for rather than spelled out -- adding TRUE 16:9 needed
 * no new branch in this function, only a new entry in AleksAspect_ExtraSide.
 */
void ZeldaSetAspect(int aspect) {
  bool enable;
  int extra;

  if (aspect < 0 || aspect >= kAleksAspect_Count)
    aspect = kAleksAspect_4x3;
  extra = AleksAspect_ExtraSide(aspect);
  enable = extra != 0;

  /* The stored aspect is corrected even when the geometry already matches, so
   * a config that says one thing and a PPU that says another cannot persist. */
  g_config.aleks_gameplay_aspect = (uint8)aspect;
  {
    /* 240-line mode is part of the aspect now, so it must be committed even
     * when the side space happens to be unchanged -- otherwise switching
     * between two aspects with the same width would leave the wrong height. */
    bool want240 = AleksAspect_Height240(aspect);
    if (g_config.extend_y != want240) {
      g_config.extend_y = want240;
      g_display_reconfig_pending = true;
    }
  }
  if (g_config.extended_aspect_ratio == extra)
    return;

  g_config.extended_aspect_ratio = (uint8)extra;
  if (enable)
    g_config.features0 |= kFeatures0_ExtendScreen64 | kFeatures0_WidescreenVisualFixes;
  else
    g_config.features0 &= ~(kFeatures0_ExtendScreen64 | kFeatures0_WidescreenVisualFixes);

  /* ZeldaRunFrame latches this into game ram at the start of the next frame --
   * the same path the QoL toggles use, so no direct ram poke here. */
  g_wanted_zelda_features = g_config.features0;

  if (g_zenv.ppu) {
    g_zenv.ppu->extraLeftRight = extra;
    /* Clamp the CURRENT side space to the new maximum in the same breath.
     * PpuSetExtraSideSpace does the UintMin against extraLeftRight, so this
     * can never leave a Cur value larger than the frame now is. */
    PpuSetExtraSideSpace(g_zenv.ppu, g_zenv.ppu->extraLeftCur,
                         g_zenv.ppu->extraRightCur, g_zenv.ppu->extraBottomCur);
    /* Leaving Wide must not carry a widescreen camera offset into the 4:3
     * frame; the tracker below is reset for the same reason. */
    if (extra == 0)
      g_zenv.ppu->renderObjXOffset = 0;
  }
  /* The tracker's bounds were computed against the old margin. */
  memset(&g_fixed_camera_tracker, 0, sizeof(g_fixed_camera_tracker));
  g_display_reconfig_pending = true;
}

bool ZeldaConsumeDisplayReconfig(void) {
  bool pending = g_display_reconfig_pending;
  g_display_reconfig_pending = false;
  return pending;
}
...
void ZeldaDrawPpuFrame(uint8 *pixel_buffer, size_t pitch, uint32 render_flags) {
  ...
  FixedCameraRenderState fixed_camera = BeginFixedCameraRender();

  if (g_zenv.ppu->extraLeftRight != 0 || render_flags & kPpuRenderFlags_Height240) {
    ConfigurePpuSideSpace(fixed_camera.visual_x, fixed_camera.uses_visual_camera,
                          fixed_camera.horizontal_transition);
  } else {
    /* extraLeftRight is 0, so there is no side space -- but the PPU's CURRENT
     * side space must be told that, or it keeps rendering the previous frame's
     * width.  Skipping this call is what made Wide -> 4:3 write past the end of
     * the narrower texture. */
    PpuSetExtraSideSpace(g_zenv.ppu, 0, 0, 0);
  }
  ...
  EndFixedCameraRender(&fixed_camera);
}
...
/* Drop the autosave slot.  Called on a deliberate Save & Exit so the next
 * launch restores the save the player just made rather than resurrecting the
 * autosave taken moments earlier.  (Esteban; absent from Samyost.) */
