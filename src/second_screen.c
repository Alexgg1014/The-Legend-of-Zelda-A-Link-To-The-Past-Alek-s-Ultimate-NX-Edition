...
static bool g_second_screen_initialized;

void SecondScreen_Init(void) {
  g_second_screen_initialized = true;
}

bool SecondScreen_IsInitialized(void) {
  return g_second_screen_initialized;
}

void SecondScreen_GetState(SecondScreenState *state) {
  if (!state) return;
  state->module = SS_GetModule();
  state->area = SS_GetArea();
  state->dungeon = SS_GetDungeon();
  state->link_x = SS_GetLinkX();
  state->link_y = SS_GetLinkY();
  state->equipped_slot = SS_GetEquippedSlot();
  state->rupees = link_rupees_actual;
  state->bombs = link_bomb_filler;
  state->arrows = link_num_arrows;
  state->indoors = SS_IsIndoors();
  state->dark_world = savegame_is_darkworld != 0;
}
...
/* Link's current facing, as the engine records it: 0 up, 2 down, 4 left,
 * 6 right (link_direction_facing).  Reported as 0..3 up/down/left/right, or
 * -1 when there is nothing meaningful to report. */
int SS_GetLinkFacing(void) {
  switch (link_direction_facing & 6) {
  case 0: return 0;   /* up    */
  case 2: return 1;   /* down  */
  case 4: return 2;   /* left  */
  case 6: return 3;   /* right */
  }
  return 1;
}

/* Link's face in the mail he is actually wearing.
 *
 * kPalette_ArmorAndGloves holds the armour palettes back to back, 15 colours
 * each: green, blue, red.  link_armor indexes them, so the marker tracks the
 * player's mail for the cost of an offset -- no palette subsystem.
 *
 * Per-direction Link SPRITES are deliberately not used: kLinkGraphics is
 * addressed by the engine through kLinkDmaSources1/2 (SNES bank addresses for
 * the per-frame DMA), not as a statically addressable sheet of standing
 * frames, so picking "the up-facing tile" would be guesswork.  The companion
 * shows the authentic face and the renderer draws the facing indicator around
 * it (see draw_overworld) -- authentic art, and a direction that cannot be
 * wrong. */
bool SS_RenderLinkFaceArmor(uint32 *px, int chunk) {
  if (!SS_AssetsReady()) return false;
  const uint8 *gfx = kLinkGraphics;
  int mail = link_armor <= 2 ? link_armor : 0;
  const uint16 *pal = kPalette_ArmorAndGloves + mail * 15;
  memset(px, 0, 16 * 16 * 4);
...
// Deferred: these mutate gameplay, HUD, saves, settings, or desktop controls.
// Requested from the UI thread; applied on the game thread at frame start.
static volatile int g_pending_equip_slot;
static volatile int g_pending_use_slot;   /* Quick Item: USE, never equip */
...
static volatile int g_pending_controls_reset;
...
/*
 * Does the player actually own the item in this inventory-grid slot?
 *
 * Lives here, next to the queueing it guards, rather than in the UI: input
 * code asks for a slot and the backend decides whether that is a legal
 * request.  Same rule the companion's ITEMS page paints with (slot 16 is the
 * bottle cell, whose count lives at 0x4F), read from the same game RAM.
 */
bool SS_CanEquipSlot(int slot) {
  int i;
  if (slot < 1 || slot > 20) return false;
  i = slot - 1;
  return (i == 15 ? g_ram[0xF300 + 0x4F] : g_ram[0xF300 + 0x40 + i]) > 0;
}

/* Returns false when the request was refused, so a caller can give feedback
 * instead of assuming it worked.  Never equips inventory the player lacks. */
bool SS_EquipSlot(int slot) {
  if (!SS_CanEquipSlot(slot)) return false;
  g_pending_equip_slot = slot;
  return true;
}

/*
 * QUICK USE, as opposed to SS_EquipSlot's QUICK EQUIP.
 *
 * Queued like everything else that touches the engine, and applied on the game
 * thread.  The frontend only ever names a slot; every rule about whether that
 * item can actually be used right now -- ownership, ammo, magic, Link's state,
 * the module -- belongs to the game's own item code, which this ends up
 * calling.  Nothing here decrements a counter or writes SRAM.
 */
bool SS_RequestUseItemSlot(int slot) {
  if (slot < 1 || slot > 20) return false;
  if (!SS_CanEquipSlot(slot)) return false;   /* not owned: safe no-op */
  g_pending_use_slot = slot;
  return true;
}
...
/* g_pending_widescreen carries an ASPECT now (kAleksAspect_*), not a flag;
 * -1 still means idle, and 0/1 happen to be the two values it used to hold,
 * so the queue's contract is unchanged for existing callers. */
void SS_SetWidescreen(bool on) {
  g_pending_widescreen = on ? kAleksAspect_Wide : kAleksAspect_4x3;
}

void SS_SetAspect(int aspect) {
  if (aspect < 0 || aspect >= kAleksAspect_Count) return;
  g_pending_widescreen = aspect;
}

int SS_GetAspect(void) {
  int pending = g_pending_widescreen;
  if (pending >= 0) return pending;
  /* No request in flight: report what the engine actually is, derived from
   * the live side space rather than from the stored setting, so the row can
   * never show an aspect the PPU is not drawing. */
  if (g_zenv.ppu)
    return AleksAspect_FromExtraSide(g_zenv.ppu->extraLeftRight);
  return g_config.aleks_gameplay_aspect;
}

/* Esteban wide-camera edge mode, applied on the game thread like the rest. */
static volatile int g_pending_edge_mode = -1;

void SS_SetWideEdgeMode(int mode) {
  g_pending_edge_mode = mode == 1 ? 1 : 0;
  g_config.aleks_wide_camera = (uint8)(mode == 1 ? 1 : 0);
}

int SS_GetWideEdgeMode(void) {
  int pending = g_pending_edge_mode;
  return pending >= 0 ? pending : ZeldaGetWidescreenEdgeMode();
}

/* Kept so the donor second-screen UI links unchanged.  The 3DS-only display
 * plumbing has no Switch counterpart -- NORMAL/DUAL/FLIP and the aspect are
 * owned by aleks_layout.c and SS_SetWidescreen -- and a restart is served by
 * the engine's own reset rather than a platform hook. */
void SS_Set3DSDisplayMode(int mode) { (void)mode; }
void SS_Set3DSWideEdgeMode(int mode) { (void)mode; }
...
bool SS_IsCrtFilter(void) {
  ...
  return false;  // Switch renderer has no independent CRT filter setting.
}
...
/* NULL restores the stock bindings -- the settings menu's RESET DEFAULTS. */
void SS_SetGamepadControls(const int *in) {
  if (!in) {
    g_pending_controls_reset = 1;
    return;
  }
  for (int i = 0; i < 12; i++)
...
/* HUD mode.  AUTO hides the gameplay HUD whenever a companion is on screen,
 * because the companion already shows the same status -- the reason the donor
 * has the option at all.  ON/OFF are explicit overrides.
 *
 * Applied on the game thread rather than on the companion's draw, so it also
 * takes effect in NORMAL, where the companion is not drawing. */
static void ApplyHudMode(void) {
  bool hide;
  switch (g_config.aleks_hud_mode) {
  case 1:  hide = false; break;
  case 2:  hide = true;  break;
  default: hide = g_config.aleks_display_mode != 0; break;
  }
  if (SS_IsHudHidden() != hide)
...
void SecondScreen_RunFrameHook(void) {
  ...
  int edge = g_pending_edge_mode;
  if (edge >= 0) {
    g_pending_edge_mode = -1;
    ZeldaSetWidescreenEdgeMode(edge);
  }
  ...
  if (g_pending_controls_reset) {
    g_pending_controls_reset = 0;
    GamepadMap_ResetControls();
  } else if (g_pending_controls_set) {
  ...
  /* Quick USE.  Same slot -> HUD item conversion the equip path uses, and the
   * same in_gameplay gate, so a Quick Item can never fire in a menu, a
   * cutscene or a transition.  Link_HandleYItem consumes it on this frame and
   * the item's own code decides everything after that; no icon refresh,
   * because the equipped item has not changed. */
  slot = g_pending_use_slot;
  if (slot) {
    g_pending_use_slot = 0;
    if (in_gameplay)
      g_aleks_quick_use_hud_item =
          hud_inventory_order[0] ? hud_inventory_order[slot - 1] : (uint8)slot;
  }
}
